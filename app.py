...
Streamlit + Folium interactive map of predicted lake states for Northern Ontario communities and lakes.
...
from mlp_inference import load_keras_mlp
...
# ---------------------------------------------------------------------------
# Community data — Northern Ontario First Nations communities and lakes (lat/lon ± ~1 km accuracy)
# ---------------------------------------------------------------------------
COMMUNITIES = [
    ...
    {"name": "Ginoogaming First Nation",            "lat": 49.801, "lon": -86.701, "lake": "Long Lake"},
     {"name": "God's Lake", "lat": 54.725, "lon": -94.175, "lake": "God's Lake"},
    {"name": "Island Lake", "lat": 53.800, "lon": -94.675, "lake": "Island Lake"},
    {"name": "Oxford Lake", "lat": 55.000, "lon": -95.450, "lake": "Oxford Lake"},
    {"name": "Red Sucker Lake", "lat": 54.075, "lon": -93.950, "lake": "Red Sucker Lake"},
    {"name": "South Indian Lake", "lat": 56.975, "lon": -98.600, "lake": "South Indian Lake"},
    {"name": "Granville Lake", "lat": 56.450, "lon": -100.000, "lake": "Granville Lake"},
    {"name": "Knee Lake", "lat": 54.575, "lon": -95.375, "lake": "Knee Lake"},
    {"name": "Moose Lake", "lat": 53.850, "lon": -100.150, "lake": "Moose Lake"},
]
...
# Load thresholds in cm for different vehicle types
LOAD_THRESHOLDS_CM = {
    "Light truck (2.5-ton gross)": 8 * 2.54,
    "Medium truck (3.5-ton gross)": 10 * 2.54,
    "Heavy truck (8-ton gross)": 12 * 2.54,
    "10 tons": 15 * 2.54,
    "25 tons": 20 * 2.54,
    "70 tons": 30 * 2.54,
    "110 tons": 36 * 2.54,
...
def add_load_assessment(df: pd.DataFrame, vehicle_type: str) -> pd.DataFrame:
    df = df.copy()

    threshold_cm = LOAD_THRESHOLDS_CM[vehicle_type]

    df["vehicle_type"] = vehicle_type
    df["required_ice_cm"] = threshold_cm
    df["load_margin_cm"] = df["ice_thickness_cm"] - threshold_cm

    df["load_status"] = np.where(
        df["ice_thickness_cm"] >= threshold_cm,
        "Safe",
        np.where(
            df["ice_thickness_cm"] >= 0.85 * threshold_cm,
            "Borderline",
            "Unsafe"
        )
    )
...
@st.cache_resource
def load_user_model_artifacts():
    """Load the reduced MLP used by the sidebar predictor (no TensorFlow)."""
    user_model = load_keras_mlp(MODEL_DIR / "final_mlp5_model.keras")
    user_X_scaler = joblib.load(MODEL_DIR / "feature_scaler_05.pkl")
    user_y_scaler = joblib.load(MODEL_DIR / "target_scaler_05.pkl")
    return user_model, user_X_scaler, user_y_scaler
...
def predict_ice_depth(model, X_scaler, y_scaler, feature_names, feature_dict):
    """Predict ice depth in cm using a trained MLP model."""
    X = pd.DataFrame([feature_dict])
    X = X[list(feature_names)]
    X_scaled = X_scaler.transform(X)
    pred_scaled = model.predict(X_scaled).flatten()
    pred_m = y_scaler.inverse_transform(
        pred_scaled.reshape(-1, 1)
    ).flatten()[0]
    return max(0.0, pred_m) * 100


def ice_depth_to_state(ice_cm):
    """Convert predicted ice depth to lake state."""
    if ice_cm >= 30:
        return "Frozen"
    elif ice_cm >= 15:
        return "Unstable"
...
def ice_depth_to_risk(ice_cm):
    """Higher risk when ice is thinner."""
    return round(100 - min(100, (ice_cm / 60) * 100), 1)


def estimate_road_days(state):
    return {"Frozen": 70, "Unstable": 25, "Open": 5}[state]
...
def fetch_all_weather(today_iso: str) -> dict:
    """Fetch daily forecast + history for all communities in parallel."""
    def _one(c):
        try:
            r = requests.get(
...
def build_map(df: pd.DataFrame, date: datetime.date, scenario: str,
    ...
    m = folium.Map(location=center, zoom_start=zoom, tiles="CartoDB positron") #Esri.WorldImagery"

    for _, row in df.iterrows():
        is_focused = bool(focus_community and row["name"] == focus_community)
        color = LOAD_STATUS_COLOR[row["load_status"]]
        emoji = LOAD_STATUS_EMOJI[row["load_status"]]
        ...
        popup_html += f"""
            <tr><td>Selected load</td><td align="right"><b>{row['vehicle_type']}</b></td></tr>
            <tr><td>Required ice</td><td align="right"><b>{row['required_ice_cm']:.1f} cm</b></td></tr>
            <tr><td>Load status</td><td align="right"><b>{row['load_status']}</b></td></tr>
            <tr><td>Margin</td><td align="right"><b>{row['load_margin_cm']:.1f} cm</b></td></tr>"""
...
with st.sidebar:
    ...
    selected_vehicle = st.selectbox(
        "Vehicle / Load Type",
        list(LOAD_THRESHOLDS_CM.keys()),
        index=0
    )

    st.caption(
        f"Minimum ice thickness required: "
        f"**{LOAD_THRESHOLDS_CM[selected_vehicle]:.1f} cm**"
    )

    st.caption(
        "Load thresholds are planning indicators only. "
        "Multiple vehicles, spacing, speed, ice quality, cracks, currents, and local authority guidance must also be considered."
    )
    ...
    wi_snow = st.slider("Snow Depth (cm)", 0, 150, 50, 5,
                        format="%d cm")
    wi_previous_cum_fdd = st.slider(
        "Previous Cumulative Freezing Degree-Days",
        0, 600, 200, 10,
        format="%d FDD",
        help="This represents previous cold accumulation for the selected lake.",
    )

    wi_fdd = max(0, -wi_temp)
    wi_cum_fdd = wi_previous_cum_fdd + wi_fdd
    wi_sqrt_cum_fdd = np.sqrt(wi_cum_fdd)

    try:
        user_model, user_X_scaler, user_y_scaler = load_user_model_artifacts()
        user_features = user_X_scaler.feature_names_in_
        user_feature_dict = dict(
            zip(user_features, user_X_scaler.mean_.astype(float))
        )
        user_feature_dict.update({
            "sd": wi_snow,
            "wind_speed": wi_wind,
            "sqrt_cum_fdd": wi_sqrt_cum_fdd,
            "t2m_c": wi_temp,
        })
        wi_ice_cm = predict_ice_depth(
            user_model,
            user_X_scaler,
            user_y_scaler,
            user_features,
        ...
        wi_state = ice_depth_to_state(wi_ice_cm)
        wi_risk = ice_depth_to_risk(wi_ice_cm)
    except Exception as exc:
        st.error(f"Could not run ice prediction model: {exc}")
        wi_state = ice_depth_to_state(0)
        wi_risk = 100.0
        wi_ice_cm = 0.0
    ...
    st.markdown(
        ...
        f'Ice depth: {wi_ice_cm:.1f} cm · Risk: {wi_risk:.1f}/100</span>'
        f'</div>',
...
st.markdown(
    ...
    f"Predicted lake states for {len(COMMUNITIES)} communities and lakes"
)
...
if realtime_active:
    ...
    if n_failed:
        st.warning(
            f"{n_failed} of {len(COMMUNITIES)} communities fell back to scenario data "
...
df = add_load_assessment(df, selected_vehicle)
...
display_cols = {
    ...
    "ice_thickness_cm": "Ice (cm)",
    "required_ice_cm": "Required Ice (cm)",
    "load_margin_cm": "Margin (cm)",
    "road_open_days": "Est. Road Days",
}
...
def highlight_status(row):
    color = BG.get(row["Load Status"], "")
    return [f"background-color:{color}" for _ in row]


styled = table_df.style.apply(highlight_status, axis=1).format(
    {
        "Risk Score": "{:.1f}",
        "Ice (cm)": "{:.1f}",
        "Required Ice (cm)": "{:.1f}",
